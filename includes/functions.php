...
function createAccount($pUsername, $pPassword) { 
  ...
  if (!empty($pUsername) && !empty($pPassword)) { 
    ...
    $sql = "SELECT Login_Name FROM webusermaster WHERE Login_Name = '" . $eUsername . "' LIMIT 1"; 
    ...
    /*if ($uLen <= 4 || $uLen >= 11) { 
      $_SESSION['error'] = "Login_Name must be between 4 and 11 characters."; 
    }elseif ($pLen=4) { 
      $_SESSION['error'] = "Password must be longer then 6 characters."; 
    }else*/if (mysql_num_rows($query) == 1) { 
      $_SESSION['error'] = "Login_Name already exists."; 
    }else { 
      // All errors passed lets 
      // Create our insert SQL by hashing the password and using the escaped Username. 
      $sql = "INSERT INTO webusermaster (`Login_Name`,`Password`) VALUES ('".$eUsername."','".$pPassword."');"; 
...
function loggedIn() { 
  ...
  if (isset($_SESSION['loggedin']) && isset($_SESSION['Login_Name'])) { 
    return true; 
  } 
...
function validateUser($pUsername, $pPassword) { 
  ...
  $sql = "SELECT Login_Name FROM webusermaster  WHERE Login_Name = '" . mysql_real_escape_string($pUsername) . "' AND Password = '" . mysql_real_escape_string($pPassword) . "' LIMIT 1"; 
  $query = mysql_query($sql) or trigger_error("Query Failed: " . mysql_error()); 
